...
def edit_czml_objects(request):
    ...
    if request.method == "POST":
        ...
        if data["function"] == "search":
            ...
            objects_filtered = (
                objects.filter(name__icontains=keyword).order_by("name")
                if keyword != ""
                else objects.order_by("name")
            )
            ...
            objectsN_filtered = (
                objectsN.filter(name_presentation__icontains=keyword)
                .order_by("name")
                .exclude(pk__in=object_ids)
                if keyword != ""
                else objectsN.order_by("name").exclude(pk__in=object_ids)
            )
            object_idsN = set(objectsN_filtered.values_list("pk", flat=True))

            rooms = (
                Room.objects.filter(name__icontains=keyword)
                if keyword != ""
                else Room.objects.none()
            )
            room_objects = (
                CzmlObject.objects.filter(
                    pk__in=rooms.values_list("object_father", flat=True)
                )
                .exclude(pk__in=object_ids)
                .exclude(pk__in=object_idsN)
            )
            final_objects = (
                list(objects_filtered) + list(room_objects) + list(objectsN_filtered)
            )
            final_objects = list(set(final_objects))  # Eliminar duplicados
            final_objects = sorted(
                final_objects, key=lambda x: x.name
            )  # Ordenar por nombre
            objects_dict = [
                {
                    "id": obj.id_object,
                    ...
                    "category_object": (
                        obj.category_object.pk if obj.category_object else None
                    ),
                    "count_rooms": len(
                        Room.objects.filter(object_father=obj.id_object)
                    ),
                    "rooms": [
                        {
                            "id": room.pk,
                            "name": room.name,
                            "type_room": (
                                room.type_room.name if room.type_room else None
                            ),
                            "floor": room.floor,
                            "url": room.image_room.url if room.image_room else None,
                            ...
                            "location": (
                                {
                                    "latitud": (
                                        ...
                                        if room.location
                                        else obj.location.latitude
                                    ),
                                    "longitud": (
                                        room.location.longitude
                                        if room.location
                                        else obj.location.longitude
                                    ),
                                    "altitud": (
                                        room.location.altitude
                                        if room.location
                                        else obj.location.altitude
                                    ),
                                }
                                if room.location
                                else None
            ...
            return JsonResponse(
                {
                    "status": "success",
                    "created": created,
                    "message": f'Objeto {obj.name} {"creado" if created else "actualizado"}',
                }
            )
    if request.method == "PUT":
        ...
        try:
            ...
            obj.save()
            return JsonResponse(
                {"status": "success", "message": f"Objeto {obj.name} actualizado"}
            )
        except CzmlObject.DoesNotExist:
            return JsonResponse(
                {"status": "error", "message": "Objeto no encontrado"}, status=404
            )
    if request.method == "DELETE":
        ...
        try:
            ...
            obj.delete()
            return JsonResponse(
                {"status": "success", "message": f"Objeto {obj.name} eliminado"}
            )
        except CzmlObject.DoesNotExist:
            return JsonResponse(
                {"status": "error", "message": "Objeto no encontrado"}, status=404
...
def edit_nodes(request):
    ...
    if request.method == "GET":
        ...
        objects_dict = objects_dict = objects_dict = [
            {
                "id": obj.id_object,
                "name": obj.name,
                "category_node_pk": obj.category_node.pk,
                "category_node_name": obj.category_node.name,
                "latitud": obj.latitude,
                "longitud": obj.longitude,
                "altitud": obj.altitude,
            }
            for obj in objects
        ]
        return JsonResponse({"objects": objects_dict})
    ...
    if request.method == "POST":
        ...
        if data["function"] == "search":
            keyword = data["keyword"]
            nodes_filtered = (
                Node.objects.filter(name__icontains=keyword)
                .exclude(category_node__pk=1)
                .exclude(category_node__name__in=["Camino", "Conjunto"])
                .order_by("category_node__name", "name")
                if keyword != ""
                else Node.objects.all()
                .exclude(category_node__pk=1)
                .exclude(category_node__name__in=["Camino", "Conjunto"])
                .order_by("category_node__name", "name")
            )
            nodes_dict = [
                {
                    "pk": node.pk,
                    "name": node.name,
                    "category_node_pk": node.category_node.pk,
                    "category_node_name": node.category_node.name,
                    "latitud": node.latitude,
                    "longitud": node.longitude,
                    "altitud": node.altitude,
                }
                for node in nodes_filtered
            ]
            return JsonResponse({"nodes": nodes_dict})
        if data["function"] == "save":
            obj, created = Node.objects.update_or_create(
                pk=data["pk"],
                defaults={
                    "name": data["Nombre"],
                    "category_node": Category.objects.get(pk=data["Categoria_nodo"]),
            ...
            return JsonResponse(
                {
                    "status": "success",
                    "created": created,
                    "message": f'Nodo {obj.name} {"creado" if created else "--actualizado"}',
                }
            )
    if request.method == "PUT":
        data = json.loads(request.body)
        ...
        try:
            obj.name = data["Nombre"]
            obj.category_node = Category.objects.get(id=data["Categoria_nodo"])
            obj.latitude = data["Latitud"]
            obj.longitude = data["Longitud"]
            obj.altitude = data["Altitud"]
            obj.save()
            return JsonResponse(
                {"status": "success", "message": f"Nodo {obj.name} actualizado"}
            )
        except Node.DoesNotExist:
            return JsonResponse(
                {"status": "error", "message": "Nodo no encontrado"}, status=404
            )
    ...
    if request.method == "DELETE":
        try:
            ...
            node.delete()
            return JsonResponse(
                {"status": "success", "message": f"Nodo {node.name} eliminado"}
            )
        except Node.DoesNotExist:
            return JsonResponse(
                {"status": "error", "message": "Nodo no encontrado"}, status=404
    ...
    if request.method == "POST":
        ...
        if not Edge.objects.filter(start_node=end_node, end_node=start_node).exists():
            obj, created = Edge.objects.update_or_create(
                ...
                end_node=end_node,
                defaults={"distance": data["Distancia"]},
            )
            return JsonResponse(
                {
                    "status": "success",
                    "created": created,
                    "message": f'Camino {obj.start_node.pk} - {obj.end_node.pk} {"creado" if created else "actualizado"}',
                }
            )
        return JsonResponse(
            {
                "status": "success",
                "message": f"Camino {obj.start_node} - {obj.end_node} ya existe",
            }
        )
    if request.method == "PUT":
        data = json.loads(request.body)
        ...
        if obj:
            ...
            return JsonResponse(
                {
                    "status": "success",
                    "message": f"Camino {obj.start_node.pk} - {obj.end_node.pk} actualizado",
                }
            )
    ...
    if request.method == "DELETE":
        ...
        try:
            ...
            return JsonResponse(
                {
                    "status": "success",
                    "message": f"Camino {obj.start_node.pk} - {obj.end_node.pk} eliminado",
                }
            )
        except Edge.DoesNotExist:
            return JsonResponse(
                {"status": "error", "message": "Camino no encontrado"}, status=404
            )
            ...
            with transaction.atomic():
                for start_node in node_ids:
                    lengths, paths = nx.single_source_dijkstra(
                        G, source=start_node, weight="weight"
                    )
                    ...
                    for end_node, total_distance in lengths.items():
                        ...
                        for i, node_id in enumerate(path):
                            RouteNode.objects.create(
                                route=route_ab, node_id=node_id, order=i
                            )
                        ...
                        for i, node_id in enumerate(reversed(path)):
                            RouteNode.objects.create(
                                route=route_ba, node_id=node_id, order=i
                            )
        ...
        if data["function"] == "get":
            ...
            if route.exists():
                ...
                return JsonResponse(
                    {
                        ...
                        "message": f"Ruta de {start_node.name} a {end_node.name} encontrada",
                        "status": "success",
                    }
                )
            else:
                return JsonResponse(
                    {
                        "status": "error",
                        "message": "Ruta no encontrada, debes verificar los caminos",
                    },
                    status=404,
                )
