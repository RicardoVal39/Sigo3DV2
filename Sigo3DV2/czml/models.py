...
class Category(models.Model):
    ...
    Las categorias representan los diferentes tipos de elementos usados en Unillanos3D.

    Attributes:
        name (str): Nombre de la categoria.
        icon (image): Icono de la categoria.
        context (str): Contexto de la categoria (nodos, objetos_czml, salones, usuarios, horarios, recursos).
    """

    context_choices = [
        ("nodos", "Nodos"),
        ("objetos_czml", "Objetos_Czml"),
        ("salones", "Salones"),
        ("usuarios", "Usuarios"),
        ("horarios", "Horarios"),
        ("recursos", "Recursos"),
    ...
    name = models.CharField(
        max_length=255, verbose_name="Nombre de la categoria", default="Camino"
    )  # Nombre de la categoria
    icon = models.ImageField(
        upload_to="icons_category/",
        verbose_name="Icono de la categoria",
        null=True,
        blank=True,
...
class Node(models.Model):
    ...
    Attributes:
        ...
        category_node (Category): Categoria del nodo.
        latitude (float): Latitud del punto.
        longitude (float): Longitud del punto.
        altitude (float): Altitud del punto (en metros).
    """

    name = models.CharField(
        max_length=255, verbose_name="Nombre del punto"
    )  # Nombre del punto
    category_node = models.ForeignKey(
        Category,
        related_name="category_node",
    ...
    altitude = models.FloatField(
        verbose_name="Altitud", default=0
    )  # Altitud del punto (en metros)
...
class CzmlObject(models.Model):
    ...
    Attributes:
        ...
        name_presentation (str): Nombre de la presentación del objeto.
        type_object (str): Tipo de objeto (por ejemplo, "Modelo").
        description (str): Descripción del objeto CZML.
        is_activated (bool): Indica si el objeto está activado o no.
        location (Node): Ubicación del objeto en el mapa.
        heading (float): Rotación Y del objeto.
        pitch (float): Rotación X del objeto.
        roll (float): Rotación Z del objeto.
        scale (float): Escala del objeto.
        image_object (image): Imagen del objeto CZML.
        category_object (Category): Categoria del objeto CZML.
    """

    id_object = models.CharField(max_length=100, primary_key=True, default="0000")
    name = models.CharField(
        max_length=100, verbose_name="Nombre del objeto CZML"
    )  # Nombre del objeto CZML
    name_presentation = models.CharField(
        max_length=100, verbose_name="Nombre de la presentacion", null=True, blank=True
    )  # Nombre de la presentacion
    type_object = models.CharField(
        max_length=100, verbose_name="Tipo de objeto", default="Modelo"
    )  # Tipo de objeto
    description = models.TextField(
        verbose_name="Descripción del objeto", null=True, blank=True
    )  # Descripción del objeto
    is_activated = models.BooleanField(
        default=False, verbose_name="Activado"
    )  # Indica si el objeto está activado
    location = models.ForeignKey(
        "Node",
        related_name="location_czml",
        on_delete=models.CASCADE,
    ...
    image_object = models.ImageField(
        upload_to="images_czml/",
        verbose_name="Imagen del objeto",
        null=True,
        blank=True,
    )  # Imagen del objeto
    category_object = models.ForeignKey(
        Category,
        related_name="category_object",
        on_delete=models.CASCADE,
        verbose_name="Categoria del objeto",
        null=True,
        blank=True,
    ...
    def __str__(self):
        return f"{self.id_object} - {self.name}"  # Return the name of the object
...
class Edge(models.Model):
    ...
    start_node = models.ForeignKey(
        ...
        on_delete=models.CASCADE,
        verbose_name="Nodo inicial",
    )  # Nodo inicial
    end_node = models.ForeignKey(
        Node,
        related_name="incoming_edges",
        on_delete=models.CASCADE,
        verbose_name="Nodo final",
    )  # Nodo final
    distance = models.FloatField(
        verbose_name="Distancia"
    )  # Distancia entre los nodos, calculada en base a sus coordenadas

    class Meta:
        verbose_name = "Caminos"
...
class RouteNode(models.Model):
    """
    Modelo para los nodos que conforman las rutas, guardando su posicion y orden.

    Attributes:
        route (Route): Ruta a la que pertenece el nodo.
        node (Node): Nodo de la ruta.
        order (int): Orden del nodo en la ruta.
    """

    route = models.ForeignKey(
        "Route", on_delete=models.CASCADE, verbose_name="Ruta a la que pertenence"
    )
    node = models.ForeignKey(
        "Node", on_delete=models.CASCADE, verbose_name="Nodo de la ruta"
    )
