...
function firstVisionRoute(path) {
    ...
    // Armar path_coordinates como [tiempo, lon, lat, alt]
    ...
    // Calcular tangentes
    // const firstTangent = Cesium.Cartesian3.subtract(points[0], before, new Cesium.Cartesian3());
    // const lastTangent = Cesium.Cartesian3.subtract(after, points[points.length - 1], new Cesium.Cartesian3());
    ...
    const entity = viewer.entities.add({
        ...
        viewFrom: new Cesium.Cartesian3(-5, 0, 5),
    });

    viewer.trackedEntity = entity;
    zoomToCoordinates(path_coordinates[-1][2], path_coordinates[-1][1], path_coordinates[-1][3]);
}
function generarRutas(){
    let html ='';
    console.log("Generando rutas...");
    info('Generando rutas...',html,"fullscreen","proces");
    $.ajax({
        type: 'POST',
        headers: {
            'X-CSRFToken': getCSRFToken()
        ...
        success: function(response){
            ...
            $('#modalInfo').hide();
            $('#modalInfo').removeClass('fullscreen');
        },
        error: function(error){
            console.log(error);
        }
